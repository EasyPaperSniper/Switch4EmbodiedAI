...
import scripts.utils.lafan_vendor.utils as utils
...
def load_gvhmr_pred_file(gvhmr_pred_file, smplx_body_model_path):
    ...
    # GVHMR outputs 10 betas (SMPL standard), no need to pad
    ...
    smplx_data = {
        ...
        'betas': betas.numpy() if torch.is_tensor(betas) else betas,
    ...
    body_model = smplx.create(
        ...
        gender="neutral",
        use_pca=False,
        num_betas=10,  # GVHMR uses 10 betas (SMPL standard)
        num_expression_coeffs=10,  # Standard expression coefficients
    )
